"""Deterministic numbered SQL migrations."""
...
from projectctl.db import connect, journal_mode
from projectctl.paths import DEFAULT_DB_PATH, MIGRATIONS_DIR
...
def list_migration_files(migrations_dir: Path | None = None) -> list[Path]:
    ...
    files = [p for p in directory.iterdir() if p.is_file() and _MIGRATION_RE.match(p.name)]
...
def apply_migrations(
    ...
    Returns the list of newly applied migration version names.
    """
    ...
    try:
        # Establish WAL on initialization / migration runs.
        ...
        # Confirm WAL after migration commit.
